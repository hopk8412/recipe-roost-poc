<script lang="ts">
	...
	// ─── Ingredient list state (initialised from existing recipe) ──────────────
	type Ingredient = { name: string; quantity: string; unit: string };
	let ingredientsList = $state<Ingredient[]>(
		(() => {
			try {
				const p = JSON.parse(data.form.data.ingredientsJson ?? '[]') as Ingredient[];
				return p.length > 0 ? p : [{ name: '', quantity: '', unit: '' }];
			} catch {
	...
	// ─── Step list state (initialised from existing recipe) ───────────────────
	type Step = { instruction: string };
	let stepsList = $state<Step[]>(
		(() => {
			try {
				const p = JSON.parse(data.form.data.stepsJson ?? '[]') as Step[];
				return p.length > 0 ? p : [{ instruction: '' }];
			} catch {
				return [{ instruction: '' }];
			}
		})()
...
<svelte:head>
	<title>Edit: {data.recipe.title} — Recipe Roost</title>
</svelte:head>
...
<div class="mx-auto max-w-2xl space-y-8">
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="mt-1 text-sm text-gray-500">{data.recipe.title}</p>
		</div>
		<a href="/recipes/{data.recipe.id}" class="text-sm text-gray-500 hover:text-gray-700">
			← Back to recipe
		</a>
	</div>
			...
			<div>
				...
				{#if data.recipe.imageUrl}
					<img
						...
						alt="Current cover"
						class="mt-2 mb-3 h-32 w-auto rounded-lg object-cover"
					/>
					<p class="mb-2 text-xs text-gray-400">Upload a new image to replace the current one.</p>
				{/if}
				<input
					id="image"
					name="image"
					type="file"
					accept="image/*"
					class="block w-full text-sm text-gray-500 file:mr-4 file:rounded-lg file:border-0 file:bg-orange-50 file:px-4 file:py-2 file:text-sm file:font-medium file:text-orange-700 hover:file:bg-orange-100"
				/>
			</div>
		</section>
